...
import { Modal, Box, TextField, IconButton, Avatar, MenuItem, Select, FormControl, InputLabel } from '@mui/material';
import supabase from '../../config/supabaseClient';
import CustomButton from '../../components/Button/Button';
import CloseIcon from '@mui/icons-material/Close';
import styles from './EditGroupModalPost.module.scss';
...
const EditGroupModalPost: React.FC<EditGroupModalProps> = ({ open, handleClose, groupId }) => {
    ...
    if (groupId) {
      const fetchGroupDetails = async () => {
        try {
          ...
          if (groupData?.project_number != null) {
            const { data: projectData, error: projectError } = await supabase
              .from('projects')
              .select('project_id')
              .eq('project_number', groupData.project_number.toString())
              .single();

            if (projectError) {
              console.error('Error fetching project details:', projectError);
            } else {
              setSelectedProject(projectData?.project_id || '');
            }
          }
        } catch (error) {
    ...
    const fetchProjects = async () => {
      try {
        const { data: projectsData, error: projectsError } = await supabase
          .from('projects')
          .select('project_id, title, project_number');
        ...
        const sortedProjects: SimpleProject[] = (projectsData || []).sort(
  ...
  const handleSave = async () => {
    try {
      ...
      if (projectError ?? !projectData) {
        console.error('Error fetching project number:', projectError);
        return;
      }
      ...
      const { error: groupError } = await supabase
        .from('groups')
        .update({ name: groupTitle, project_number: projectNumber })
        .eq('group_id', groupId);

      if (groupError) {
        console.error('Error updating group project number:', groupError);
        return;
      }

      const { error: allocationError } = await supabase
        .from('allocations')
        .upsert({ group_id: groupId, project_id: selectedProject }, { onConflict: 'group_id' })
        .eq('group_id', groupId);

      if (allocationError) {
        console.error('Error updating allocation project ID:', allocationError);
        return;
      }

      const { error: unallocatedError } = await supabase.from('unallocated_groups').delete().eq('group_id', groupId);

      if (unallocatedError) {
        console.error('Error removing group from unallocated_groups:', unallocatedError);
        return;
      }

      const notifications = members.map((member) => ({
        user_id: member.auth_id,
        body: `P${projectNumber} allocated to ${groupTitle}`,
        read: false,
      }));
      ...
      <Box
        ...
        <div className={styles.header}>
          <Box display='flex' alignItems='center' sx={{ margin: 0 }}>
            <h1 style={{ marginRight: '16px', fontSize: '40px' }}>Project:</h1>
...
export default EditGroupModalPost;
